...
load('datar/All_mdr_approx.Rdata')                #ARI data - with approx to all countries having regional levels
...
load('datar_approx/uu.Rdata') # unique countries
...
for(i in 1:length(cnz)){ # for all the countries
    ...
    if(lin > 0) {fn <- paste0('datar_approx/',cn,'.Rdata')}# grab the ARI data 
    if(lin == 0) {fn <- paste0('datar_const_approx/',cn,'.Rdata')}
    ...
    if(lin > 0) {fn <- paste0('datar_approx/zz_',cn,'.Rdata')}# grab the ARI data 
    if(lin == 0) {fn <- paste0('datar_const_approx/zz_',cn,'.Rdata')}
...
## ---- Approximate by putting all at whoregion new MDR level ----- #### 
