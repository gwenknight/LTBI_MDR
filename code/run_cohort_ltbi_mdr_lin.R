...
### COMBINING MDR AND DS CURVES
# ## Read in MDR curves
# all_samples0 <- read.csv("~/Dropbox/MDR/output/all_samples_1970_2014.csv")[,-1]
# 
...
# load("~/Dropbox/MDR/data/rundata_ari_1000.Rdata")
# 
# rundatar$ari <- exp(rundatar$lari)

# 
# ## MERGE DS AND MDR
# all0 <- merge(all_samples0, rundatar, by = c("year","iso3","replicate"))
# # MDR-ARI
# all0$mdr_ari <- all0$prediction * all0$ari
#
# TOTAL AND MDR
# all0$ds_ari <- all0$ari - ari$mdr_ari # ds ARI is the remainder of the ari

# save(all0,"~/Dropbox/MDR/data/all0_ds_mdr.Rdata")

# READ IN
load("~/Dropbox/MDR/data/all0_ds_mdr.Rdata")
...
write.csv(level2014, paste0("~/Dropbox/MDR/output/level2014_",nari,".csv"))

s_level0 <- s_level
s_level$pop_name <- cni[as.numeric(s_level0$popf)]
s_level <- as.data.table(s_level)
write.csv(s_level, paste0("~/Dropbox/MDR/output/s_level_",nari,".csv"))
...
a2r
save_plot(paste0("~/Dropbox/MDR/output/ltbi_all_countries_r",nari,".pdf"), a2r, base_aspect_ratio = 1.5 )
...
a2s
save_plot(paste0("~/Dropbox/MDR/output/ltbi_all_countries_s",nari,".pdf"), a2s, base_aspect_ratio = 1.5 )
...
a2r<-ggplot(s_level_mean, aes(x=pop_name, y = ltbir )) + geom_point() + 
  ...
  theme(axis.text.y = element_text(size = 3),axis.text.y = element_text(size = 6)) + 
  coord_flip() + aes(x=reorder(pop_name,ltbir),y=ltbir) +
  geom_errorbar(aes(min = ltbir.lo, max = ltbir.hi))
a2r
save_plot(paste0("~/Dropbox/MDR/output/ltbi_all_countries_r_mean",nari,".pdf"), a2r, base_aspect_ratio = 0.8)
...
colors = c("#F1EEF6", "#D4B9DA", "#C994C7", "#DF65B0", "#DD1C77", "#980043","grey")
...
pdf("~/Dropbox/MDR/output/map_ltbis.pdf")
mapParams <- mapCountryData(mapped_data, nameColumnToPlot = "ltbis", catMethod = seq(0,50,10),
                            colourPalette = cols, 
                            addLegend = FALSE)
...
pdf("~/Dropbox/MDR/output/map_ltbir.pdf")
