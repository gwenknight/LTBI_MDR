...
for(cci in 1:llu){
  ...
  save_plot(paste0("../data/",cni[cci],"_mdr_trends_with_data_",pp,".pdf"), a1, base_aspect_ratio = 2 )
  
  a2 <- ggplot(rdata, aes(x=year, y = mdr_ari, group = factor(replicate))) + geom_line() + 
    scale_y_continuous("MDR ARI") + scale_x_continuous("Year") 
  save_plot(paste0("../data/",cni[cci],"_mdr_ari_",pp,".pdf"), a2, base_aspect_ratio = 2 )
  ...
  write.csv(sa, paste0("../data/",cni[cci],"_sa_",nari,"_",pp,".csv"))
  
  # Just recent infection 
  w<-which(sa$year > 2012)
  write.csv(sa[w,], paste0("../data/",cni[cci],"_rec_infec_",nari,"_",pp,".csv"))
...
write.csv(level2014, paste0("../data/level2014_",nari,"_",pp,".csv"))

s_level0 <- s_level
s_level$pop_name <- cni[as.numeric(s_level0$popf)]
s_level <- as.data.table(s_level)
write.csv(s_level, paste0("../data/s_level_",nari,"_",pp,".csv"))

### OR READ IN
#level2014 <- read.csv("../data/level2014_10_lin.csv",stringsAsFactors = FALSE)[,-1]
#s_level <- read.csv("../data/s_level_10_lin.csv", stringsAsFactors = FALSE)[,-1]
...
a2r
save_plot(paste0("../data/ltbi_all_countries_r",nari,"_",pp,".pdf"), a2r, base_aspect_ratio = 1.5 )
...
a2s
save_plot(paste0("../data/ltbi_all_countries_s",nari,"_",pp,".pdf"), a2s, base_aspect_ratio = 1.5 )
...
a2r
save_plot(paste0("../data/ltbi_all_countries_r_mean",nari,"_",pp,".pdf"), a2r, base_aspect_ratio = 0.8)
...
a2s
save_plot(paste0("../data/ltbi_all_countries_s_mean",nari,"_",pp,".pdf"), a2s, base_aspect_ratio = 1.5 )
...
pdf(paste0("../data/map_ltbis_",nari,"_",pp,".pdf"))
mapParams <- mapCountryData(mapped_data, nameColumnToPlot = "ltbis", catMethod = seq(0,50,10),
                            colourPalette = cols,
                            addLegend = FALSE)
...
pdf(paste0("../data/map_ltbir_",nari,"_",pp,".pdf"))
...
for(cci in 1:llu){
  ...
  sa <- read.csv(paste0("../data/",cni[cci],"_sa_",nari,"_infor_prior.csv"))[,-1]
  
  # 2014 level
  level2014 <- read.csv(paste0("../data/level2014_",nari,"_infor_prior.csv"))[,-1]
  ...
  ggsave(paste0("../data/eg_all_age_",cni[cci],"_r_",labl,".pdf"), height = 10, width = 10)
  ...
  ggsave(paste0("../data/eg_all_age_",cni[cci],"_s_",labl,".pdf"), height = 10, width = 10)
  ...
  ggsave(paste0("../data/eg_all_age_",cni[cci],"_map_s_",labl,".pdf"), height = 10, width = 10)
  
  ggplot(sa_rec[which(sa_rec$mdr_rep < 5),], aes(year, age)) + 
    geom_tile(aes(fill = pr_dr),colour = "white") + 
    scale_fill_gradient(low = "white",high = "red4","MDR-TB") +
    labs(x = "Year",y = "Age") + scale_y_continuous(lim = c(0,100)) + facet_wrap(~mdr_rep)
  ggsave(paste0("../data/eg_all_age_",cni[cci],"_map_r_",labl,".pdf"), height = 10, width = 10)
  ...
  write.csv(s_new,paste0("../data/s_all_",cni[cci],"_",labl,".csv"))
  ...
  ggsave(paste0("../data/eg_DS_age_",cni[cci],"_ltbis_when_",labl,".pdf"), height = 10, width = 10)
  ...
  ggplot(ss_here[which(ss_here$mdr_rep < 5),], aes(age, pr_r, fill = factor(year))) +
    ...
    scale_fill_hue("clarity")
  ggsave(paste0("../data/eg_DR_age_",cni[cci],"_ltbir_when_",labl,".pdf"), height = 10, width = 10)
  ...
  ggplot(ss_here, aes(mdr_rep, pr_ltbis, fill = factor(yearcat))) +
    ...
    ggtitle("DS-TB") +
    scale_fill_hue("Year")
  ggsave(paste0("../data/DS_",cni[cci],"_ltbis_when_",labl,".pdf"), height = 10, width = 20)
  ...
  ggplot(ss_here,aes(mdr_rep, pr_ltbir, fill = factor(yearcat))) +
    ...
    ggtitle("MDR-TB") + 
    scale_fill_hue("Year")
  ggsave(paste0("../data/DR_",cni[cci],"_ltbir_when_",labl,".pdf"), height = 10, width = 20)
  ...
  write.csv(meanv, paste0("../data/",cni[cci],"_props_ltbi_when_",labl,".csv"))
  ...
  ggsave(paste0("../data/DR_mean_",cni[cci],"_ltbir_when_",labl,".pdf"), height = 10, width = 20)
  
  ss_mean <- rbind(ss_mean, cbind(mean_both,cni[cci],ii))
  
  write.csv(paste0("../data/",cni[cci],"_ss_mean_",nari,"_",labl,".csv"))[,-1]
