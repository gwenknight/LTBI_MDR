...
##### CAN"T DO FOR DS AND MDR: NEED TO USE COHORT MODEL!! 
...
for(ii in 1:3){ # 3 model predictions
  
  if (ii == 1){ 
    if(file.exists("~/Dropbox/MDR/output/all_samples_lin_1970_2014.csv")){
      all_samples0 <- read.csv("~/Dropbox/MDR/output/all_samples_lin_1970_2014.csv")
    }else{all_samples0 <- read.csv("output/all_samples_lin.csv")[,-1]}}
  if (ii == 2){ 
    if(file.exists("~/Dropbox/MDR/output/all_samples_quad_1970_2014.csv")){
      all_samples0 <- read.csv("~/Dropbox/MDR/output/all_samples_quad_1970_2014.csv")
    }else{all_samples0 <- read.csv("output/all_samples_quad.csv")[,-1]}}
  if (ii == 3){ 
    if(file.exists("~/Dropbox/MDR/output/all_samples_quadc_1970_2014.csv")){
      all_samples0 <- read.csv("~/Dropbox/MDR/output/all_samples_quadc_1970_2014.csv")
    }else{all_samples0 <- read.csv("output/all_samples_quadc.csv")[,-1]}}
  
  if(dim(all_samples)[1] < 6000000 ){ # if haven't added in pre-1970 zeros etc...
    # Rename
  ...
  table1 <- as.data.frame(cbind( paste0(sprintf('%.1f',100*MID$LTBI), " [", 
                                 ...
                                 paste0(sprintf('%.2f',100*MID$propkr), " [", 
                                        sprintf('%.2f',100*LO$propkr), "-", 
                                        sprintf('%.2f',100*HI$propkr),"]")
  ))
  colnames(table1) <- c("Prev_LTBI(%)","Prop_in_kids(%)","Prev_MDR-LTBI(%)", "Prop_MDR_in_kids(%)")
