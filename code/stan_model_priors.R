...
uu <- read.csv("~/Dropbox/MDR/who/107_final_list_included_countries.csv",stringsAsFactors = FALSE)[,-1]
...
for(ii in 1:luu) {
  ...
  country <- uu[ii]
  print(c(ii,as.character(country)))
  ...
  m.p <- stan(file="quadratic_priors.stan",
              ...
              pars=c("b", "c","t_m","rho", "p_pred","log_likelihood"),
