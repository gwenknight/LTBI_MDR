...
###********** Load code and data ************************************************************************************************#######
source("cohort_ltbi_mdr.R") # loads function for underyling cohort model
...
## Which countries? 
cni <- read.csv("../data/138_final_list_included_countries.csv",stringsAsFactors = FALSE)[,-1]
length(cni) # 138 now
llu <- length(cni)
...
#print(c(as.numeric(args[1]),cni[as.numeric(args[1])]))

### WHO data
d <-subset(w_data, iso3 == as.character(cni[as.numeric(args[1])]) )


if(!file.exists(paste0("../output_sa/",cni[as.numeric(args[1])],"s_level_",nari,"_",pp,".csv"))){
  ### ARI for both DS and mDR in all0
  rdata <- all0[which(all0$iso3 == as.character(cni[as.numeric(args[1])])),]
  ...
  R.devices::suppressGraphics(ggsave(paste0("../output_sa/",cni[as.numeric(args[1])],"_mdr_trends_with_data_",pp,".pdf"),width=11, height=11))
  ...
  R.devices::suppressGraphics(ggsave(paste0("../output_sa/",cni[as.numeric(args[1])],"_mdr_ari_",pp,".pdf"),width=11, height=11))
  
  for(i in 1:nari){
    #print(c(i,"ari rep"))
    ari <- rdata[which(rdata$replicate == i),c("ds_ari","mdr_ari")]
    colnames(ari) <- c("ds","mdr")
    pop <- as.data.frame(POP2014[which(as.character(POP2014$iso3) == as.character(cni[as.numeric(args[1])])),"value"])
    ...
    # Bind together.
    s_level <- rbind(s_level,c(i,ltbi_dr,ltbi_ds,as.numeric(args[1]),pltbi_dr, pltbi_ds,ltbi_dr_kids,ltbi_ds_kids, pltbi_dr_kids, pltbi_ds_kids, sum(pop), sum(pop[1:3,1])))
    
    ssc <- cc$store_c
    lowi <- ((runn-1)*(dim(ssc)[1])+1)
    uppi <- ((runn)*(dim(ssc)[1]))
    store_all[lowi:uppi,1] <- i;
    store_all[lowi:uppi,2] <- cni[as.numeric(args[1])];
    store_all[lowi:uppi,3:10] <- ssc
    
    runn <- runn + 1
    sa <- rbind(sa,cbind(i,cni[as.numeric(args[1])], ssc)) # just for this country
  }
  
  # store all for this country
  sa <- as.data.frame(sa)
  colnames(sa) <- c(c("mdr_rep","cn"),colnames(cc$store_c))
  write.csv(sa, paste0("../output_sa/",cni[as.numeric(args[1])],"_sa_",nari,"_",pp,".csv"))
  
  # Just recent infection
  w<-which(sa$year > 2012)
  write.csv(sa[w,], paste0("../output_sa/",cni[as.numeric(args[1])],"_rec_infec_",nari,"_",pp,".csv"))
  ...
  write.csv(level2014, paste0("../output_sa/",cni[as.numeric(args[1])],"level2014_",nari,"_",pp,".csv"))
  
  s_level0 <- s_level
  s_level$pop_name <- cni[as.numeric(s_level0$popf)]
  s_level <- as.data.table(s_level)
  write.csv(s_level, paste0("../output_sa/",cni[as.numeric(args[1])],"s_level_",nari,"_",pp,".csv"))
...
sa <- read.csv(paste0("../output_sa/",cni[as.numeric(args[1])],"_sa_",nari,"_infor_prior.csv"))[,-1]

# 2014 level
level2014 <- read.csv(paste0("../output_sa/",cni[as.numeric(args[1])],"level2014_",nari,"_",pp,".csv"))[,-1]
...
R.devices::suppressGraphics(ggsave(paste0("../output_sa/eg_all_age_",cni[as.numeric(args[1])],"_r_",labl,".pdf"), height = 10, width = 10))
...
R.devices::suppressGraphics(ggsave(paste0("../output_sa/eg_all_age_",cni[as.numeric(args[1])],"_s_",labl,".pdf"), height = 10, width = 10))
...
R.devices::suppressGraphics(ggsave(paste0("../output_sa/eg_all_age_",cni[as.numeric(args[1])],"_map_s_",labl,".pdf"), height = 10, width = 10))

a<-ggplot(sa_rec[which(sa_rec$mdr_rep < 5),], aes(year, age)) +
  geom_tile(aes(fill = pr_dr),colour = "white") +
  scale_fill_gradient(low = "white",high = "red4","MDR-TB") +
  labs(x = "Year",y = "Age") + scale_y_continuous(lim = c(0,100)) + facet_wrap(~mdr_rep)
R.devices::suppressGraphics(ggsave(paste0("../output_sa/eg_all_age_",cni[as.numeric(args[1])],"_map_r_",labl,".pdf"), height = 10, width = 10))
...
#l14 <- level2014[which(as.character(level2014$cn) == as.character(cni[as.numeric(args[1])])),] # don't need
l14 <- level2014
...
write.csv(s_new,paste0("../output_sa/s_all_",cni[as.numeric(args[1])],"_",labl,".csv"))
...
pop <-  POP2014[which(as.character(POP2014$iso3) == as.character(cni[as.numeric(args[1])])),"value"]
...
a<-ggplot(ss_here[which(ss_here$mdr_rep < 5),], aes(age, pr_s, fill = factor(year))) +
  ...
  scale_y_continuous() + facet_wrap(~mdr_rep) + ggtitle(paste0("DS-TB, ",cni[as.numeric(args[1])])) +
  scale_fill_hue("clarity")
R.devices::suppressGraphics(ggsave(paste0("../output_sa/eg_DS_age_",cni[as.numeric(args[1])],"_ltbis_when_",labl,".pdf"), height = 10, width = 10))

a<-ggplot(ss_here[which(ss_here$mdr_rep < 5),], aes(age, pr_r, fill = factor(year))) +
  geom_bar(position = "fill", stat = "identity") +
  scale_y_continuous() + facet_wrap(~mdr_rep) + ggtitle(paste0("MDR-TB, ",cni[as.numeric(args[1])])) +
  scale_fill_hue("clarity")
R.devices::suppressGraphics(ggsave(paste0("../output_sa/eg_DR_age_",cni[as.numeric(args[1])],"_ltbir_when_",labl,".pdf"), height = 10, width = 10))
...
a<-ggplot(ss_here, aes(mdr_rep, pr_ltbis, fill = factor(yearcat))) +
  ...
  ggtitle("DS-TB") +
  scale_fill_hue("Year")
R.devices::suppressGraphics(ggsave(paste0("../output_sa/DS_",cni[as.numeric(args[1])],"_ltbis_when_",labl,".pdf"), height = 10, width = 20))
...
a<-ggplot(ss_here,aes(mdr_rep, pr_ltbir, fill = factor(yearcat))) +
  ...
  ggtitle("MDR-TB") +
  scale_fill_hue("Year")
R.devices::suppressGraphics(ggsave(paste0("../output_sa/DR_",cni[as.numeric(args[1])],"_ltbir_when_",labl,".pdf"), height = 10, width = 20))
...
write.csv(meanv, paste0("../output_sa/",cni[as.numeric(args[1])],"_props_ltbi_when_",labl,".csv"))
...
R.devices::suppressGraphics(ggsave(paste0("../output_sa/DR_mean_",cni[as.numeric(args[1])],"_ltbir_when_",labl,".pdf"), height = 10, width = 20))

ss_mean <- rbind(ss_mean, cbind(mean_both,cni[as.numeric(args[1])]))

write.csv(ss_mean, paste0("../output_sa/",cni[as.numeric(args[1])],"_ss_mean_",nari,"_",labl,".csv"))
