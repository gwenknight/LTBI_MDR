...
####**** WHO data name errors ********************************************************************************************************************************************************************#####
## special countries -- mergers
## use SCG for MNE and for SRB
# GK: data tidy of countries where names have changed / merged over time
# GK: SCG = Serbia and Montenegro
# GK: MNE = Montenegro. 
# GK: SRB = Serbia
...
####**** Data from Cauthen ********************************************************************************************************************************************************************#####
## Cauthen 1988 Bull WHO
# Publication with data on estimated ARI, survey sample size, and mean age for 100 country-years in 24 countries
C <- read.csv('data/inputData_Cauthen_ARI.csv')
C$ari <- 1e-2*C$ARI # multiple by 0.01 to move from percentage? 
## get ages
...
####**** New data from review performed in study ********************************************************************************************************************************************************************#####
## new data ---
...
####**** Proportion HIV in TB patients ********************************************************************************************************************************************************************#####
## HIV data proportion HIV in TB
# Grab from WHO data= case detection / variation in cdr? / prevalence / variance in prevalence? 
NH <- Nro[,list(iso3,year,
              cdr = c_cdr/100,
              vcdr = (c_cdr_hi-c_cdr_lo)^2/(100*3.92)^2,
              p = e_tbhiv_prct/100,
              vp = (e_tbhiv_prct_hi-e_tbhiv_prct_lo)^2/(100*3.92)^2)]
## drop NA from % HIV
NH <- NH[!is.na(p),]
## drop <1% for o
NH <- NH[p>1e-2,]                       #2931 = dim(NH)
...
for(cn in as.character(bcnz)){          #min CDR, max var
    NH$cdr [NH$iso3==cn & is.na(NH$cdr)]  <- min(NH$cdr [NH$iso3==cn & !is.na(NH$cdr)]) # gk: if cdr is na replace with min cdr that is not na
    NH$vcdr[NH$iso3==cn & is.na(NH$vcdr)] <- max(NH$vcdr[NH$iso3==cn & !is.na(NH$vcdr)]) # gk: if cvdr is na, replace wtih max vcdr that is not na
}
NH[is.na(cdr),]                         #check: none
NH[cdr>1]$vcdr <- 0                     #cap CDR for this - gk should this be vcdr? 
NH[cdr>1]$cdr <- 1
...
## T1, T2 and variances...
# gk: see supplementary: 1 / 2 = HIV neg / pos respectively. Ti = duration of disease. Uniform distribution. Different for notified (*cdr) or un-notified (*(1-cdr))
# gk: gives the mean TB duration by HIV status for a given country-year 
...
# gk: The smear-positivity of TB in people living with HIV was assumed to be a factor f less likely = U[.3,.4]/U[.4,.5] = 0.35/0.45 = 0.777778 
mf <- 0.780
vf <- 0.00666
...
## var log S....
NH[,A:=(p*T2*mf + (1-p)*T1)] # numerator for S (fraction sm+ reduction)
NH[,B:=(p*T2 + (1-p)*T1)] # denominator for S (fraction sm+ reduction)
NH[,vlS:= vp*(p*(1-p)*(1-mf)*T1/(A*B))^2 +
       (T1^2*vT2+T2^2*vT1)*(p*(1-p)*(1-mf)/(A*B))^2 +
           vf*(p*T2/A)^2]               # varlogS/logS^2=E^2
NH[,S:= A/B]                            #S - fraction redn (The overall factor reduction S in smear-positivity for a country year)
...
## dealing with missing mFr from this merge (gk: missing data from Kunkel above - input the mean for the country)
...
## dealing with countries with no mFr data = input median from all other countries
...
## calculations
Nrc <- copy(Nr)
Nrc[,ari:=e_prev_100k*1e-5 * mFr * S * F] # ARI calculation: prevalence per capita * proportion smear positive * fraction reduction due to HIV * Styblo factor
Nrc[,lari:=log(ari)]
