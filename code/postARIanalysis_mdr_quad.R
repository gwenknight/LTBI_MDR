...
all_samples0 <- read.csv("output/all_samples_quad.csv")[,-1]
# Rename
...
rall <- rall0[order(replicate,iso3,year),
                     list(ari=lari,mdr_ari=mdr_ari,H=cumsum(lari),Hr=cumsum(mdr_ari),
                          year=year),
...
write.csv(table1, "~/Dropbox/MDR/output/table1_quad.csv")
...
write.csv(table1n, "~/Dropbox/MDR/output/table1n_quad.csv")
